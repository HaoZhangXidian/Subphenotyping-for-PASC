"""
===========================================
Beta Sampler implemented on GPU
===========================================
...
__global__ void rand_Beta(float *randomseed, float *target, int *matrix_scale, float *shape1, float*shape2)
{
    ...
    if (idx<matrix_scale[0])
    {
        ...
        // sample gamma 1
        float alpha = shape1[current_position];
        if (alpha<1)
        {
            alpha += 1.0;
            p = shape1[current_position];
        }
        else p = 1;
        ...
        {
            u = seed / 2147483647.0;
            gamma1 = float(d * pow(double(u), double(1.0 / p)));
        }
        
        // sample gamma2
        alpha = shape2[current_position];
        if (alpha<1) 
        {
            alpha += 1.0;
            p = shape2[current_position];
        }
        else p = 1;
        ...
        {
            u = seed / 2147483647.0;
            gamma2 = float(d * pow(double(u), double(1.0 / p)));
        }
        
        // sample beta
        target[idx]=gamma1/(gamma1+gamma2);
    }
}

""")

def beta(a, b, times=1, device='cpu'):

    a, b, output, matrix_scale, randomseed, partition, single_number = para_preprocess(times, np.float32, np.float32, a, b)

    func = Sampler.get_function('rand_Beta')
